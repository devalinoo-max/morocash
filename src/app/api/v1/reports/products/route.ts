...
import { getProductsReport, resolveMonthRange } from '@/server/modules/reports/queries';
...
export async function GET(request: Request) {
  try {
    ...
    const products = await getProductsReport(ctx.businessId, from, to);
    return ok({ from, to, products });
  } catch (error) {
    return fail(error);
  }
}
